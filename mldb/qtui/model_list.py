...
class ModelListWidget(QWidget):
    ...
    def do_tSNE_worker(self):
        ...
        tsne = TSNE(
            perplexity=min(len(data_enc) - 1, 5), learning_rate="auto", init="pca"
        )
        ...
        self.tSNE_data = dict(
            x=res[:, 0], y=res[:, 1], colour=0, data_enc=data_enc, keys=sorted(all_keys)
        )
        self.plot_tSNE()
    ...
    def plot_tSNE(self):
        ...
        smap = ScalarMappable(
            norm=Normalize(vmin=0, vmax=colour_data.max()), cmap="viridis"
        )
